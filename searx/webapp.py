...
from searx import oidc as oidc_module
from searx.userdb import get_userdb
...
def render(template_name: str, **kwargs):
    ...
    kwargs['current_user'] = getattr(sxng_request, 'user', None)
    kwargs['oidc_enabled'] = oidc_module.is_enabled()
...
def search():
    ...
    # save query to per-user history (HTML requests only, no bots/API consumers)
    if output_format == 'html' and search_query:
        _user = getattr(sxng_request, 'user', None)
        if _user:
            try:
                get_userdb().add_history(_user['sub'], search_query.query)
...
@app.route('/info/<locale>/<pagename>/_disabled', methods=['GET'])
def info_disabled(pagename, locale):
...
def preferences():
    ...
    # Block access when OIDC is active but user is not logged in
    if oidc_module.is_enabled() and not getattr(sxng_request, 'user', None):
...
# ---------------------------------------------------------------------------
# History suggest API (used by the search-field history dropdown)
# ---------------------------------------------------------------------------
...
def history_suggest():
    ...
    if not user:
        return jsonify([])
    entries = get_userdb().get_history(user['sub'], limit=8)
    return jsonify([e['query'] for e in entries])
...
@app.route('/login')
def login():
    if not oidc_module.is_enabled():
        flask.abort(404)
    redirect_uri = url_for('auth_callback', _external=True)
    return oidc_module.begin_login(redirect_uri)
...
def auth_callback():
    ...
    try:
        ...
        logger.exception("OIDC callback error: %s", e)
        sxng_request.errors.append(gettext('Login failed, please try again.'))
    return redirect(url_for('index'))


@app.route('/logout')
def logout():
    flask.session.pop('oidc_user', None)
    return redirect(url_for('index'))
...
def history():
    ...
    if not user:
        return redirect(url_for('login'))
    entries = get_userdb().get_history(user['sub'])
    return render('history.html', history=entries)
...
def history_clear():
    user = getattr(sxng_request, 'user', None)
    if user:
        get_userdb().clear_history(user['sub'])
...
def favorites():
    ...
    if not user:
        return redirect(url_for('login'))
    entries = get_userdb().get_favorites(user['sub'])
    return render('favorites.html', favorites=entries)
...
def favorites_add():
    user = getattr(sxng_request, 'user', None)
    if user:
        fav_url = sxng_request.form.get('url', '').strip()
        fav_title = sxng_request.form.get('title', '').strip() or fav_url
        if fav_url:
            try:
                get_userdb().add_favorite(user['sub'], fav_url, fav_title)
            except Exception:  # pylint: disable=broad-except
                pass
    referrer = sxng_request.referrer
    return redirect(referrer if referrer else url_for('favorites'))


@app.route('/favorites/remove/<int:fav_id>', methods=['POST'])
def favorites_remove(fav_id: int):
    user = getattr(sxng_request, 'user', None)
    if user:
        get_userdb().remove_favorite(user['sub'], fav_id)
...
def init():
    ...
    oidc_module.init_oidc(app)
    if oidc_module.is_enabled():
        get_userdb()  # trigger DB schema creation on startup
